...
def save_samples_labels(fname, samples, labels):
    # [N, 3]
    r = (labels).reshape([-1, 1]) * 255
    g = (1-labels).reshape([-1, 1]) * 255
    b = np.zeros(r.shape)

    to_save = np.concatenate([samples, r, g, b], axis=-1)
...
def saveobj_with_color(mesh_path, verts, faces, colors, reverse=False):
